...
function cmd(cmd) {
  if (typeof cmd !== 'object') cmd = { command: cmd, params: [] };

  let instruction = null
  let { params, executable, command } = cmd;
  ...
  if(executable || command) {
    if(params)
      instruction = `CMD ["${executable || command}", ${params.map(enQuote).join(', ')}]`
    else
...
function entryPoint(entrypoint) {
  const { executable, params } = entrypoint;
  entrypoint = [executable || ''].concat(boxAndFilter(params || []));
...
*   1) Any object with terminal property-values of string, or number type.
*/
function arg(arg, onbuild) {
  var instruction = null

  arg = flattenLabels(arg)

  if(arg.length) {
    instruction = `ARG ${arg.map(e => `${e[0]}=${mapToQuote(e[1])}`).join(' \\\n    ')}`
  } else {
    throw new errors.InstructionError('env', 'expected string or array of arguments.')
  }
